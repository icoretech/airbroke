...
async function main() {
  ...
  await Promise.all(
    Array.from({ length: numberOfProjects }, async (_, projectIndex) => {
      const i = projectIndex + 1;
      ...
      await Promise.all(
        Array.from({ length: numberOfNotices }, async (_, noticeIndex) => {
          const j = noticeIndex + 1;
