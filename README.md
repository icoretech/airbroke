...
The Render Deploy Button uses `render.yaml` (Render Blueprint). On the free tier, the template runs `yarn db:migrate` at boot (safe no-op if already applied). Set `AIRBROKE_CORS_ORIGINS` in the Render dashboard for production (this controls CORS for `/api/v3/notices` and `/api/sentry/*/envelope`).
